...
const QVector<DmcColor> &palette() {
    static const QVector<DmcColor> colors = buildPalette();
    return colors;
}
...
DmcMatch nearest(QRgb rgba) {
    return nearest(QColor::fromRgba(rgba));
}

DmcMatch nearest(const QColor &color) {
    const QVector<DmcColor> &colors = palette();
    DmcMatch match;
    if (colors.isEmpty()) return match;
